...
1.  **Top-Bar / Toolbar (Global Controls):**
    ...
    *   Integrate a **Contextual Action Bar** below or within the Toolbar that appears *only* when a tool or annotation is selected (e.g., show the `FontPicker` only when a text annotation is active).
...
2.  **Delay Canvas Redrawing:** Update the React/Zustand state that triggers the canvas redraw only *after* the resize has finished, or throttle the updates.
