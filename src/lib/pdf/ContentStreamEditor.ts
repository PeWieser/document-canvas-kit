type TokenType = 'Whitespace' | 'Comment' | 'String' | 'HexString' | 'Name' | 'ArrayStart' | 'ArrayEnd' | 'DictStart' | 'DictEnd' | 'Number' | 'Operator';
...
export const REDACTION_OVERLAP_THRESHOLD = 0.50;
...
export function tokenizeStream(data: Uint8Array): Token[] {
  ...
  const isWS = (b: number) => b === 0x00 || b === 0x09 || b === 0x0A || b === 0x0C || b === 0x0D || b === 0x20;
  const isDelim = (b: number) => b === 0x28 || b === 0x29 || b === 0x3C || b === 0x3E || b === 0x5B || b === 0x5D || b === 0x7B || b === 0x7D || b === 0x2F || b === 0x25;
  ...
  while (i < data.length) {
    ...
    if (data[i] === 0x5B) { tokens.push({ type: 'ArrayStart', raw: data.slice(i, i + 1) }); i++; continue; }
    if (data[i] === 0x5D) { tokens.push({ type: 'ArrayEnd', raw: data.slice(i, i + 1) }); i++; continue; }
...
function getCharFromBytes(bytes: number[]): string {
  ...
  if (bytes[0] === 0x5C) { // backslash '\'
    if (bytes.length > 1) {
      const next = bytes[1];
      if (next >= 0x30 && next <= 0x37) { // octal digit
        let octalStr = '';
    ...
    if (raw[k] === 0x5C) {
      ...
      if (k < end) {
        if (raw[k] >= 0x30 && raw[k] <= 0x37) {
          ...
          k++;
          if (k < end && raw[k] >= 0x30 && raw[k] <= 0x37) {
            octalStr += String.fromCharCode(raw[k]); k++;
            if (k < end && raw[k] >= 0x30 && raw[k] <= 0x37) {
              octalStr += String.fromCharCode(raw[k]); k++;
            }
          }
  ...
  for (let bIdx = startByteIdx; bIdx < parsedBytes.length; bIdx += step) {
    ...
    if (charUserBounds && charUserBounds[charIdx]) {
      ...
      const userCorners = corners.map(([cx, cy]) => {
        return [
          cx * trm[0] + cy * trm[2] + trm[4],
          cx * trm[1] + cy * trm[3] + trm[5]
        ];
      });
  ...
  while (i < tokens.length) {
    ...
    if (t.type === 'Operator') {
      ...
      if (op === 'Tj' || op === 'TJ' || op === "'" || op === '"') {
        ...
        const widthScale = Math.sqrt(tm[0]*tm[0] + tm[1]*tm[1]) * Math.sqrt(ctm[0]*ctm[0] + ctm[1]*ctm[1]);

        let j = filteredTokens.length - 1;
        while (j >= 0 && (filteredTokens[j].type === 'Whitespace' || filteredTokens[j].type === 'Comment')) {
          j--;
        }
        ...
        if (j >= 0) {
          ...
          if (op === 'Tj' || op === "'" || op === '"') {
            ...
            if (operandToken.type === 'String') {
              subTokens = redactStringTokenToTokens(operandToken, tm, ctm, fontSize, redactionRects, glyphWidths, charUserBounds);
            } else if (operandToken.type === 'HexString') {
              subTokens = redactHexStringTokenToTokens(operandToken, tm, ctm, fontSize, redactionRects, glyphWidths, charUserBounds);
            }
            ...
            if (subTokens.length > 0) {
              ...
              if (op === "'" || op === '"') {
                filteredTokens.push({ type: 'Operator', raw: new TextEncoder().encode('T*'), text: 'T*' });
            ...
            for (let k = arrayStartIndex + 1; k < j; k++) {
              ...
              if (subToken.type === 'String' || subToken.type === 'HexString') {
                ...
                if (subToken.type === 'String') {
                  subTokens = redactStringTokenToTokens(subToken, tm, ctm, fontSize, redactionRects, subGlyphWidths, subCharBounds);
                } else {
                  subTokens = redactHexStringTokenToTokens(subToken, tm, ctm, fontSize, redactionRects, subGlyphWidths, subCharBounds);
                }
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (t.type === 'Operator') {
            ...
            if (op === 'q') {
                ...
                const x1 = parseFloat(tokens[i - 6]?.text || '0'), y1 = parseFloat(tokens[i - 5]?.text || '0');
                const x2 = parseFloat(tokens[i - 4]?.text || '0'), y2 = parseFloat(tokens[i - 3]?.text || '0');
                const x3 = parseFloat(tokens[i - 2]?.text || '0'), y3 = parseFloat(tokens[i - 1]?.text || '0');
                ...
                const x = parseFloat(tokens[i - 4]?.text || '0'), y = parseFloat(tokens[i - 3]?.text || '0');
                const w = parseFloat(tokens[i - 2]?.text || '0'), h = parseFloat(tokens[i - 1]?.text || '0');
                ...
                if (currentCoords.length > 0) {
                    ...
                    currentCoords = [];
                    currentMinX = Infinity; currentMinY = Infinity; currentMaxX = -Infinity; currentMaxY = -Infinity;
                }
...
export function transformShapeObjects(tokens: Token[], shapes: ShapeObject[], dx: number, dy: number, scaleX: number, scaleY: number, centerX: number, centerY: number): Token[] {
    const result = [...tokens];
    ...
    const updateNumToken = (index: number, valDelta: number, scaleFactor: number = 1, currentBase: number = 0) => {
        if (result[index] && result[index].type === 'Number') {
            ...
            if (!isNaN(current)) {
                ...
                result[index] = { ...result[index], text: ((current - currentBase) * scaleFactor + currentBase + valDelta).toFixed(4).replace(/\.?0+$/, '') };
            }
        }
    };
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        if (t.type === 'Operator') {
            ...
            if (op === 'q') {
                ...
                if (targetIndices.has(i - 4) && targetIndices.has(i - 3) && targetIndices.has(i - 2) && targetIndices.has(i - 1)) {
                    // x, y
