...
describe('font matching and formatting QA', () => {
  ...
  it('correctly extracts rotated text blocks with precise angles, sizes, and coordinates', async () => {
    ...
    const rot120 = results.find(r => r.str === 'Rotated' && Math.abs(r.angle - (120 * Math.PI) / 180) < 0.05);
    ...
    const rot120Green = results.find(r => r.str.includes('green') && Math.abs(r.angle - (120 * Math.PI) / 180) < 0.05);
  ...
  it('correctly exports and verifies rotated replacement text and multiline offsets', async () => {
    ...
    const doc = await pdfjsLib.getDocument({ data: exportedBytes, standardFontDataUrl: path.join(__dirname, '../../../../node_modules/pdfjs-dist/standard_fonts/') }).promise;
    const page = await doc.getPage(1) as any;
